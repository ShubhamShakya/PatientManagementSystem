package com.example.pms_patientmanagementsystem;

import android.os.Bundle;
import android.widget.TextView;

import androidx.annotation.Nullable;
import androidx.appcompat.app.AppCompatActivity;

public class ViewPrescription extends AppCompatActivity {
    String mPrescription;
    TextView mViewPrescriptionClassTextView;
    @Override
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.view_prescription);

        mPrescription=getIntent().getStringExtra("prescription");
        mViewPrescriptionClassTextView=findViewById(R.id.viewPrescriptionClassTextView);
        mViewPrescriptionClassTextView.setText(mPrescription);
    }
}
